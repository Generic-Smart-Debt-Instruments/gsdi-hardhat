...
contract GSDINFT is ERC721Enumerable {
  ...
  uint96 public immutable chainId;
  ...
  function setTreasury(address payable _treasury) external onlyValidAddress(_treasury) {
    treasury = _treasury;
  }

  /// @notice Sets the governance address. Only callable by governance.
  /// @param _governance New address for governance.
  function setGovernance(address _governance) external onlyValidAddress(_governance) {
  ...
  function propose(
    ...
    id = abi.encodePacked(tokenId >> 96, chainId << 160).toUint256(0);

    metadata[id] = NFTMetadata(_maturity, _faceValue, _price, _wallet, _currency, _borrower, true);
    ...
    emit Propose(id, tokenId, _borrower, address(this));
  }
  ...
  function cancel(uint256 _id) external mustBeInProposal(_id) {
    burnProposal(_id);
  ...
  function purchase(uint256 _id) external mustBeInProposal(_id) {
  ...
  function cover(uint256 _id) external mustNotBeInProposal(_id) {
