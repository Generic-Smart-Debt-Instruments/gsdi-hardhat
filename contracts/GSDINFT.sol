...
contract GSDINFT is IGSDINFT, ERC721Enumerable {
    ...
    modifier onlyValidChainId(uint256 _id) {
        require(
            gsdiChainId(_id) == chainId,
            "GSDI chainID must be the same as GSDINFT chainID."
        );
        _;
    }
    ...
    function transferBorrower(uint256 _id, address _receiver) public override {
        require(
            metadata[_id].borrower == msg.sender,
            "Sender must be borrower"
        );
    ...
    function propose(
        ...
        require(
            _wallet.executor() == msg.sender,
            "GSDINFT: Wallet executor must be sender"
        );

        uint256 tokenId = _tokenIdTracker.current();
        id_ = abi.encodePacked(chainId, uint160(tokenId)).toUint256(0);

        metadata[id_] = NFTMetadata(
            _maturity,
            _faceValue,
            _price,
            _wallet,
    ...
    function cancel(uint256 _id)
        external
        override
        mustBeInProposal(_id)
        onlyValidChainId(_id)
    {
        require(
            msg.sender == metadata[_id].borrower,
        ...
        onlyValidChainId(_id)
    {
        ...
        if (token.allowance(msg.sender, address(this)) < price.add(fee)) {
            token.approve(
                address(this),
                price.add(fee).sub(token.allowance(msg.sender, address(this)))
            );
        }
    ...
    function cover(uint256 _id)
        external
        override
        mustNotBeInProposal(_id)
        onlyValidChainId(_id)
    {
    ...
    function seize(uint256 _id) external override onlyValidChainId(_id) {
        ...
        require(
            metadata[_id].maturity <= block.timestamp,
